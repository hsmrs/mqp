...
void Husky::tagCallback(const ar_track_alvar::AlvarMarkers::ConstPtr& msg)
{
...
void Husky::updatedTaskCallback(const hsmrs_framework::TaskMsg::ConstPtr& msg){
...
void Husky::poseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg){
...
void Husky::handleRoleAssign(const hsmrs_framework::RoleMsg::ConstPtr& msg){
...
Husky::Husky(std::string name, double speed) : NAME(name), REGISTRATION_TOPIC("/hsmrs/robot_registration"), IMAGE_TOPIC("camera/rgb/image_mono"),
		...
		AUCTION_TOPIC("/hsmrs/auction"), CLAIM_TOPIC("/hsmrs/claim"), TASK_PROGRESS_TOPIC("progress"), ROLE_TOPIC("/hsmrs/role_assign")
{
	...
	//Husky publishers and subscribers
	vel_pub = n.advertise<geometry_msgs::Twist>(VEL_TOPIC, 100);
	pose_sub = n.subscribe(IN_POSE_TOPIC, 1, &Husky::poseCallback, this);
	//laser_sub = n.subscribe(LASER_TOPIC, 1000, &Husky::laserCallback, this);
	tag_sub = n.subscribe(MARKER_TOPIC, 1000, &Husky::tagCallback, this);

	bidPub = n.advertise<hsmrs_framework::BidMsg>(AUCTION_TOPIC, 100);
	claimPub = n.advertise<hsmrs_framework::BidMsg>(CLAIM_TOPIC, 100);
	
	bidSub = n.subscribe(AUCTION_TOPIC, 1000, &Husky::handleBids, this);
	newTaskSub = n.subscribe(NEW_TASK_TOPIC, 100, &Husky::handleNewTask, this);
	claimSub = n.subscribe(CLAIM_TOPIC, 100, &Husky::handleClaims, this);
	taskProgressSub = n.subscribe(TASK_PROGRESS_TOPIC, 100, &Husky::handleProgress, this);

    roleSub = n.subscribe(ROLE_TOPIC, 100, &Husky::handleRoleAssign, this);
	...
	while (ros::ok()){
		...
		{           
            ...
            boost::thread claimer = boost::thread(&Husky::claimWorker, this, *nextTask->toMsg(), nextTask->getID());
            claimer.detach();
		}
...
void Husky::handleNewTask(const hsmrs_framework::TaskMsg::ConstPtr& msg)
{
    ROS_INFO("New task");
    //boost::mutex::scoped_lock listLock(listMutex);
    std::unique_lock<std::recursive_mutex> listLock(listMutex);
...
void Husky::handleClaims(const hsmrs_framework::BidMsg::ConstPtr& msg)
{
...
void Husky::claimWorker(hsmrs_framework::TaskMsg taskMsg, int id)
{
...
{    
    ...
    if(auctionList.count(id) == 0)
    {
        ...
        //spawn claimer thread
        boost::thread claimer = boost::thread(&Husky::claimWorker, this, msg->task, id);
        claimer.detach();
    }
    else
    {
...
double Husky::bid(const hsmrs_framework::TaskMsg::ConstPtr& msg)
{
...
void Husky::handleProgress(const std_msgs::String::ConstPtr& msg)
{
...
int main(int argc, char **argv) {
	std::string name = "husky"; //default
	double speed = 1;
    
	ros::init(argc, argv, name);

	Husky* robot = new Husky(name, speed);
	return 0;
}
