...
#define MAX_AUTO_TURN (.55841/1.405935)
...
void controlCallback(const sensor_msgs::Joy::ConstPtr& joy)
{
    static bool teleopMode = true;
    
    if(joy->buttons[0] == 1) teleopMode = true;
    else if(joy->buttons[1] == 1) teleopMode = false;
    ...
    if(teleopMode)
    {
	    ...
	    {
		    vel = getPowerForWheel(wheel, x, y, r)*MAX_VEL;
	...
	{
	    double x = std::max(std::min(.1, (double)((markerX/markerZ)/MAX_AUTO_TURN)*fabs((markerX/markerZ)/MAX_AUTO_TURN)), -.1);
	    double y = std::max(std::min(.25, (double)markerZ/8), 0.0);
	    double r = std::max(std::min(.1, (double)((markerX/markerZ)/MAX_AUTO_TURN)*fabs((markerX/markerZ)/MAX_AUTO_TURN)), -.1);
	    ...
	    else if(fabs(ros::Time::now().toSec()) - lastMarkerTime > .2)
	    {
	        y = 0;
	        r = 0;
	        /*
...
void tagCallback(ar_track_alvar::AlvarMarkersConstPtr markerList)
{
    for(int i = 0; i < markerList->markers.size(); i++)
    {
        ...
        if(marker.id == 0)
        {
            ...
            markerZ = marker.pose.pose.position.z;
