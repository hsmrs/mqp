...
void Hermes::tagCallback(const ar_track_alvar::AlvarMarkers::ConstPtr& msg)
{
...
void Hermes::updatedTaskCallback(const hsmrs_framework::TaskMsg::ConstPtr& msg){
...
void Hermes::poseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg){
...
void Hermes::handleRoleAssign(const hsmrs_framework::RoleMsg::ConstPtr& msg){
		...
		AUCTION_TOPIC("/hsmrs/auction"), CLAIM_TOPIC("/hsmrs/claim"), TASK_PROGRESS_TOPIC("progress"), ROLE_TOPIC("/hsmrs/role_assign")
{
	...
	//Hermes publishers and subscribers
	vel_pub = n.advertise<geometry_msgs::Twist>(VEL_TOPIC, 100);
	pose_sub = n.subscribe(IN_POSE_TOPIC, 1, &Hermes::poseCallback, this);
	//laser_sub = n.subscribe(LASER_TOPIC, 1000, &Hermes::laserCallback, this);
	tag_sub = n.subscribe(MARKER_TOPIC, 1000, &Hermes::tagCallback, this);

	bidPub = n.advertise<hsmrs_framework::BidMsg>(AUCTION_TOPIC, 100);
	claimPub = n.advertise<hsmrs_framework::BidMsg>(CLAIM_TOPIC, 100);
	
	bidSub = n.subscribe(AUCTION_TOPIC, 1000, &Hermes::handleBids, this);
	newTaskSub = n.subscribe(NEW_TASK_TOPIC, 100, &Hermes::handleNewTask, this);
	claimSub = n.subscribe(CLAIM_TOPIC, 100, &Hermes::handleClaims, this);
	taskProgressSub = n.subscribe(TASK_PROGRESS_TOPIC, 100, &Hermes::handleProgress, this);

    roleSub = n.subscribe(ROLE_TOPIC, 100, &Hermes::handleRoleAssign, this);
	...
	while (ros::ok()){
		...
		{           
            ...
            boost::thread claimer = boost::thread(&Hermes::claimWorker, this, *nextTask->toMsg(), nextTask->getID());
            claimer.detach();
		}
...
void Hermes::handleNewTask(const hsmrs_framework::TaskMsg::ConstPtr& msg)
{
    ROS_INFO("New task");
    //boost::mutex::scoped_lock listLock(listMutex);
    std::unique_lock<std::recursive_mutex> listLock(listMutex);
...
void Hermes::handleClaims(const hsmrs_framework::BidMsg::ConstPtr& msg)
{
...
void Hermes::claimWorker(hsmrs_framework::TaskMsg taskMsg, int id)
{
...
{    
    ...
    if(auctionList.count(id) == 0)
    {
        ...
        //spawn claimer thread
        boost::thread claimer = boost::thread(&Hermes::claimWorker, this, msg->task, id);
        claimer.detach();
    }
    else
    {
...
double Hermes::bid(const hsmrs_framework::TaskMsg::ConstPtr& msg)
{
...
void Hermes::handleProgress(const std_msgs::String::ConstPtr& msg)
{
...
int main(int argc, char **argv) {
	std::string name = "hermes"; //default
	double speed = 1;
    
	ros::init(argc, argv, name);

	Hermes* robot = new Hermes(name, speed);
	return 0;
}
